...
function AdminEvents() {
  ...
  if (isLoading || !data) return <div className="flex justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div>;
        ...
        <tbody>
          {data.events.map((e) => (
            <tr key={e.id} className="border-t border-border/60">
              <td className="px-5 py-3 text-muted-foreground">{format(new Date(e.created_at), "MMM d, HH:mm")}</td>
              ...
              <td className="px-5 py-3 text-xs text-muted-foreground">{e.packet_id?.slice(0, 8) ?? "—"}</td>
            </tr>
          ))}
        </tbody>
      </table>
